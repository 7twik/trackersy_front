...
import { SimpleGrid, Box, Stack, Button,FormLabel, Input, NumberInput, NumberInputField } from '@chakra-ui/react';
            ...
            <Stack spacing={2}>
                ...
                {(noteIns!=null)?noteIns.map((noteItem)=>{
                        return(<SimpleGrid className={styles.in} columns={5} spacing={1} key={noteItem._id}>
                                    <Box key={noteItem._id.slice(0,5)} className={styles.in} bg='gray.50' height='80px'>{noteItem.Type}</Box>
                                    <Box key={noteItem._id.slice(0,6)} className={styles.in} bg='gray.50' height='80px'>{noteItem.Desc}</Box>
                                    <Box key={noteItem._id.slice(0,9)} className={styles.in} bg='gray.50' height='80px'>{noteItem.Amt['$numberDecimal'].toLocaleString()}</Box>
                                    <Box key={noteItem._id.slice(0,4)} className={styles.in} bg='gray.50' height='80px'>{noteItem.createdAt.slice(0,10)}</Box>
                                    <Box key={noteItem._id.slice(0,8)} className={styles.in} bg='gray.50' height='80px'><Button id={noteItem._id} colorScheme='teal' onClick={editFunc}><AiFillEdit /></Button> &nbsp;&nbsp; <Button className={styles.edit} colorScheme='teal'  id={noteItem._id} onClick={deleteFunc}><AiFillDelete value={noteItem} /></Button></Box>
                               </SimpleGrid>);
                }):<></>}
                <SimpleGrid columns={2} spacing={1}>
                    <Box className={styles.head} bg='teal' height='80px'>Total Expense :</Box>
